...
{
    public:
        static const char* Note_name_as_Cstring[109];


    enum Note_name {
        // name = MIDI number
                                                                       undefined_frequency = 0, A0 = 21, Bb0 = 22, B0 = 23,
...
private:
 Note_name name;
 rhythmic_value value;
 int id;
 double moment_of_birth;
 double length_in_sec;
 double length_in_eights;
 void set_length_in_sec(double s){length_in_sec = s;}
 void set_length_in_eights(double e){length_in_eights = e;}
 void set_rhytmic_value(double r){length_in_eights = r;}



 // set  double  length_in_sec l_s, length_in_eights double l_8

        Note() = default;
        Note(Note_name n,rhythmic_value r, int id, double brth): name{n},value{r},id{id},moment_of_birth{brth}{}
        Note(Note_name n): name{n} {}

    public:

        virtual ~Note();
        std::string get_Note_name() const;
        std::string get_rhythmic_value() const;
        bool operator==(Note n);            //are names equal?
friend class Sheet;
